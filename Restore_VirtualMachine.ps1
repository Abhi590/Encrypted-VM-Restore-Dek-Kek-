...
#         .Synopsis                                                                     #
#         .Description                                                                  #
#       The script will restore encrypted virtual machine with (KEK and DEK) from       #
...
$dekUrl = $obj.'properties.storageProfile'.osDisk.encryptionSettings.diskEncryptionKey.secretUrl
$dekVaultId = $obj.'properties.storageProfile'.osDisk.encryptionSettings.diskEncryptionKey.sourceVault.id
$kekUrl = $obj.'properties.storageProfile'.osDisk.encryptionSettings.keyEncryptionKey.keyUrl
$kekVaultId = $obj.'properties.storageProfile'.osDisk.encryptionSettings.keyEncryptionKey.sourceVault.id
$isenabled = $obj.'properties.storageProfile'.osDisk.encryptionSettings.enabled
...
Switch ($qresult) {
    0 {
	    ...
	    try 
		{
		    Set-AzureRmVMOSDisk -VM $vm -ManagedDiskId $osDisk.Id -DiskEncryptionKeyUrl $dekUrl -DiskEncryptionKeyVaultId $dekVaultId -KeyEncryptionKeyUrl $kekUrl -KeyEncryptionKeyVaultId $kekVaultId -CreateOption "Attach" -Windows
    ...
    1 {
	    ...
	    try 
		{
		    Set-AzureRmVMOSDisk -VM $vm -Name $osDiskName -VhdUri $osVhdUri -DiskEncryptionKeyUrl $dekUrl -DiskEncryptionKeyVaultId $dekVaultId -KeyEncryptionKeyUrl $kekUrl -KeyEncryptionKeyVaultId $kekVaultId -Caching $caching -CreateOption "Attach" -Windows -StorageAccountType $storageType
